...
from predictor.train.graphchat_trainer import GraphChatTrainer
...
# conversation_lib.SeparatorStyle.TWO
def preprocess_v1(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
) -> Dict:
...
# conversation_lib.SeparatorStyle.MPT 
def preprocess_mpt(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
) -> Dict:
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.graph_cfg = graph_cfg # graph_config
...
def train():
    ...
    model.config.pretrain_graph_model_path = os.path.join(model.config.pretrain_graph_model_path, model_args.graph_tower)
    model.config.use_cache = False

    print(model.config.pretrain_graph_model_path)
    ...
    if model_args.graph_tower is not None:
        ...
        if model_args.tune_graph_mlp_adapter: # tune_graph_mlp_adapter=True 
            model.requires_grad_(False)
            if data_args.is_graph:
