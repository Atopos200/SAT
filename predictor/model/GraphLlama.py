...
from predictor.model.graph_layers import MPNN, GNN, CLIP, graph_transformer
...
class GraphLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if hasattr(config, "graph_tower"):
            ...
            elif config.graph_tower[:8] == "clip_gcn": 
            ...
            elif config.graph_tower[:7] == "clip_gt":
                clip_graph, args= load_model_pretrained(CLIP, config.pretrain_graph_model_path) 
    ...
    def initialize_graph_modules(self, graph_tower, graph_select_layer,
        ...
        if not hasattr(self, 'graph_tower'):
            ...
            elif self.config.graph_tower[:8] == "clip_gcn": 
            ...
            elif self.config.graph_tower[:7] == "clip_gt":
                clip_graph, args= load_model_pretrained(CLIP, self.config.pretrain_graph_model_path) 
        ...
        if pretrain_graph_mlp_adapter is not None: # stage1=None, stage2=stage_1_projector.bin
    ...
    def forward(
        ...
        orig_embeds_params = getattr(self, 'orig_embeds_params', None) # List[]
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            if type(graph_data) is list:
                ...
                raise ValueError(f'graph_node_reps is expected to be a list but got {type(graph_data)}')
            tmp_dim = node_forward_out.shape[1]
            dummy_graph_features = torch.zeros(256, tmp_dim, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
        ...
        model_inputs.update(
            {
                ...
                "attention_mask": attention_mask,
                "graph_data": [kwargs.get("graph_data")] if kwargs.get("graph_data", None) else None,
